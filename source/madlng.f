     ...
     &                 ,asa)
c-----------------------------------------------------------------------
c     Calculate constants needed for the calculation of the madelung
c     potential. Optimum value of 'eta' (in units of 2*pi/a) for the
c     Ewalt sum is used in this program.
c
c     eta:    convergence parameter
c     vc:     unit cell volume (in units of a*3)
c     atmicp: position of atoms in the unit cell (in units of a)
c     natm:   number of atoms in the unit cell
c     rpt:    neighbouring lattice points (in units of a)
c     nrpt:   number of the lattice points given in 'rpt'
c     gpt:    reciprocal lattice vectors (in units of 2*pi/a)
c     ngpt:   number of the reciprocal vectors given in 'gpt'
c     rmt:    muffin-tin radius of the i-th atom (in units of a)
...
c     revised so as to also fit to ASA cases.
...
c     --- additional term coming from k=0 and that from r=0 ---
      if(i .eq. j) then
c     --- 4*eta*Q_i/sqrt(pi) => 4*(2*pi/a)*eta*Q_i/sqrt(pi)
c                             =(Q_i/a)*8*sqrt(pi)*eta
   ...
   10 continue
c     --- in the case od ASA, muffin-pan potential does not appear
c         and hence Madelung potential is given by the above.
      if(asa) return
c     write(6,1000)eta,amdlng(1,1)
c1000 format(' eta, b (see p.h.d. note) =',f12.5,f15.9)
c
c     --- in the case of muffin-tin potential model, the average
c         potential of the interstitial region have to be subtracted
c         to make the potnetial there be zero.
...
c     --- up to this point the artificial charges that include the
c         background ahrges have been used for convenience.
c         Incidentally, this causes no effects in the case od ASA
c         because background chrage is zero.
c
c     --- finally convert amdlng into another symmetric matrix which
c         is more useful. see h.a. note.
c     --- For complete description, see note by H.A, Aug. 12, 2021.
c         The point is that now the Madelung potential is given in
c         terms of total charge withing each muffin-tin sphere, not
c         one that includes the homgeneoud background charges.
      do 80 i=1,natm
      sum=0d0
      do 90 k=1,natm
