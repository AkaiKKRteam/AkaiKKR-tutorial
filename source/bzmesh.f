...
c     subroutine bzmesh(nf,nk,g,r,protat,isymop,iwk,ls)
c-----------------------------------------------------------------------
...
c     parameter(zero=1d-20, tiny=1d-10, ibit=31, nk1x=12000)
      parameter(zero=1d-20, tiny=1d-10, ibit=31)
      real*8 g(3,3),r(3,3),vkp(3,nkmx),wtkp(nkmx),protat(9,24,2)
     &      ,t(3),s(3),bc(3,3),bctyp(3)
      integer isymop(24,2),nc(0:ibit-1)
      integer*4 iwk(0:8*nf**3/ibit+1)
...
c
c     --- if ls=.false., the time reversal symmetry exists,
c         while if isymop(1,2)=1, the inversion symmetry exists.    
      if((ls .and. isymop(1,2) .eq. 0) .or. fbz) then
c     if(ls .and. isymop(1,2) .eq. 0) then
...
c         fit the unrotated mesh anymore.
      ...
      nfa=max(fct*ga+1d-1,1d0)
      nfb=max(fct*gb+1d-1,1d0)
      nfc=max(fct*gc+1d-1,1d0)
      endif
...
c     --- First generate full BZ from g-vectors.
      do 10 jc=1,2*nfc
      ic=jc-nfc
c     --- While jc runs starting from 0 to 2*nf-1, ic runs
c         from -nf0 to nf-1.
      c=dc*dble(ic)
      xc=c*g(1,3)
      yc=c*g(2,3)
...
c     --- Then mark all the equivalent k-points.
      ...
      iga=floor(ga/da+5d-1)
      igb=floor(gb/db+5d-1)
      igc=floor(gc/dc+5d-1)
c     --- when not ls, inversion is assumed irrespective of actual
...
c         unity. If not, something wrong must have happened.
   ...
   40 ichkwt=ichkwt+iwt(k)
      ...
      if(abs(chkwt-1d0) .lt. tiny) return
      deallocate(iwt)
      nfa=nf
      nfb=nf
      nfc=nf
  150 write(*,'(3x,a,i3)')'nf reset: nfa=nfb=nfc=',nf
