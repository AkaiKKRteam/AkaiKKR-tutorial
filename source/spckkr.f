     ...
     &                 ,lmxtyp,mxlcmp,msiz,lmxblk,iatmp,itblk,its,fbz)
c-----------------------------------------------------------------------
c     ------------------------------------
...
c     complex*16,allocatable::gi(:),gm(:),det(:),tcpr(:,:,:,:)
      complex*16,allocatable::gi(:),gm(:),det(:),tmp1(:,:),tmp2(:,:)
     &          ,tcpr(:,:,:,:)
c
      real*8     tchc(ng,mxl**2,ncmpx),tchs(ng,mxl**2,ncmpx)
     &          ,cm(ng,mxl**2,ncmpx)
     &          ,prr(np,nk),hh(np,(2*mxl-1)**2,nk)
     &          ,fcs(4,mxl**2,ncmpx),clks(last)
     &          ,wtkp(nk),conc(ncmpx)
c
...
c    &    ,dmpc0/1d0/, itrmx/100/, nesitr/.false./
...
c       10     not used
c
      allocate(convr2(kmx),tcpr(mxl**2,mxl**2,natm,kmx))
      call clrarc(str,mxl**2*ncmpx*kmx)
      call clrarc(phase,mxl**2*ncmpx*kmx)
      pi=4d0*atan(1d0)
      nk1=nk-nk3
      unit=2d0*pi/a
      mmxl=mxl**2
...
c
   ...
   10 call prntcs(tchc(1,1,i),tchs(1,1,i),fcs(1,1,i)
     &           ,ew,ez,ng,mxlcmp(i),201,sra)
      endif
c
      do 20 i=1,ncmpx
      do 30 k=1,kmx
      call cgntcs(e(k),ew,ez,tc(1,k),ng)
   30 call cpshft(e(k),tchc(1,1,i),tchs(1,1,i),cm(1,1,i)
     &          ,fcs(1,1,i),g(1,i,k),tc(1,k),mxl,mxlcmp(i),ng,t(1,i,k)
     &          ,pf(1,1,i,k),str(1,i,k),sra)
c
...
c     the larger the number of threads, the smaller the 'length'.
c     Even length=1 works rather well.
      lastln=mod(nk1*kmx-1,length)+1
      nvec=(nk1*kmx-lastln)/length+1
c     write(*,'(a,i3,a,i3,a,i3)')'length=',length,'  nvec=',nvec
c    &      ,'  lastln=',lastln
      allocate(tmp1(mxl**4*natm*kmx,nvec),tmp2(kmx,nvec))
      do 34 k=1,kmx
      convrg(k)=.false.
...
c     which will be used in the calculation of the phase.
      ...
      call equarc(tcpa(1,1,1,k),tcpr(1,1,1,k),mmxl**2*natm)
c---  Since the structural Green's function is constructed with the
c     real harmonics based representation while the t-matrix and
c     the local Green's function is constructed assumining the
c     spherical harmonics based one when the spin-orbit coupling
c     is taken into account. To over come this difference, the
c     t-matrix and tcpa should be transformed into real harmonics
c     representation.
      if(ls) then
      do 210 i=1,natm
      mxla=lmxtyp(itype(i))+1
  210 call gmtrns(tcpr(1,1,i,k),1,mxla,mxla,mmxl,1,1,'s2r',uu,mxl)
      endif
      call equarc(tcpr(1,1,1,k),ck(1,1,1,k),mmxl**2*natm)
      do 43 i=1,natm
   ...
   41 continue
      call clrarc(tmp1,mmxl**2*natm*kmx*nvec)
      call clrarc(tmp2,kmx*nvec)
c---  loop over vectors composed of k-mesh and e-mesh.
      snor=0d0
!$omp parallel default(shared) private(nrun,ibsadr,gm,gi,det,mmx)
      allocate(gi(msiz**2*length),gm(msiz**2*length),det(length))
c!$omp do reduction(+:ff) reduction(-:detl) reduction(+:snor)
!$omp do
      do 42 ivec=1,nvec
...
c---  construct kkr matrix
      call kkrsed(gm,gi,e,tcpr,vc,unit,np,tch,pexf,prr,hh,tc,gfree
     &           ,clks,ng,ncub,iblk,itblk,its,natm,last,mxl,nd,nk
     &           ,kmx,nrun,ibsadr,ess,korder,convrg,lmxblk,iatmp
     &           ,lmxtyp,itype,msiz)
...
c---  take k-summation
      call bzmsmb(gi,det,tmp1(1,ivec),tmp2(1,ivec),wtkp,nk,mxl,natm,kmx
c     call bzmsmb(gi,det,ff,detl,wtkp,nk,mxl,natm,kmx
     &           ,ibsadr,nrun,snor,korder,convrg,lmxtyp,itype,iatmp
     &           ,msiz)
   42 continue
!$omp end do
      deallocate(gi,gm,det)
!$omp end parallel
      call bzmsmf(ff,detl,tmp1,tmp2,nvec,mxl,natm,kmx,convrg,lmxtyp
     &              ,itype)
      snor=1d0
c     snor=snor/dble(kmx)
      if(abs(snor-1d0) .gt. 1d-6)
     &    call errtrp(2,'spckkr','weight not summed up to unity')
c---  rotate ff and sum them up.
c     The normalization ff -> ff/snor will be also taken.
      if(.not. fbz) call bzmrot(ff,natm,irotat,urotat,isymop,kmx
     &             ,convrg,mxl,snor,lmxtyp,itype,ls)
c--- Now the local Green'd function is transformed bakc to the
c    spehrical harmonics representation when the spin-orbit
c    couping is considered.
      if(ls) then
      do 220 k=1,kmx
      if(.not. convrg(k)) then
      do 230 i=1,natm
      mxla=lmxtyp(itype(i))+1
  230 call gmtrns(ff(1,1,i,k),1,mxla,mxla,mmxl,1,1,'r2s',uu,mxl)
      endif
  220 continue
      endif
...
c     --- ATA(average t-matrix approximation) iteration algorithm used.
     ...
     &           ,itype,convrg,mxl,alloy,lmxtyp)
      ...
      if(.not. fbz) call bzmrot(tcpa,natm,irotat,urotat,isymop,kmx
     &             ,convrg,mxl,1d0,lmxtyp,itype,ls)
...
c     do 62 k=1,kmx
c  62 write(*,'(i3,10f12.7)')k,(-dimag(str(l,1,k))/pi,l=1,9)
...
c     ------ print out detl -------
      ...
      write(*,1300)(k,e(k),detl(k),k=1,kmx)
 1300 format(/3x,'k,e(k),detl(k)'/(1x,i3,3x,2f12.7,2x,2f12.7))
      endif
c
...
!$omp&   private(nrun,ibsadr,gm,gi,det,iadr,kadr,nadr,k)
      allocate(gi(msiz**2*length),gm(msiz**2*length),det(length))
!$omp do
...
c---  construct kkr matrix
      call kkrsed(gm,gi,e,tcpr,vc,unit,np,tch,pexf,prr,hh,tc,gfree
     &           ,clks,ng,ncub,iblk,itblk,its,natm,last,mxl,nd,nk
     &           ,kmx,nrun,ibsadr,ess,korder,convr2,lmxblk,iatmp
     &           ,lmxtyp,itype,msiz)
c---  take the inverse of the kkr matrix
